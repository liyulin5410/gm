...
function searchList() {
    Ext.Ajax.request({
        url: "http://" + window.location.host + "/manageSign/get",
        method: 'GET',
...
function insert() {
    ...
    function submit() {
        ...
        Ext.Ajax.request({
            url: "http://" + window.location.host + "/manageSign/new",
            method: 'POST',
            params: args,
            success: function (response) {
                var json = Ext.JSON.decode(response.responseText);
...
function drawTable(dataFromServer) {
    ...
    Ext.define('LogDataStruct', {
        ...
        fields: [
            {name: 'ManageSignName', type: 'string'},
            {name: 'ManageSignMonth', type: 'string'},
            {name: 'ManageSignService', type: 'string'},
            {name: 'ManageSignDitch', type: 'string'},
            {name: 'ManageSignSatus', type: 'string'}
        ]
    });
    ...
    function initLogList() {
        var logList = Ext.create('Ext.grid.Panel', {
            ...
            height: 700,
            columns: [
                {text: '名称', width: 60, dataIndex: 'AnnouncementList'},
                {text: '日期', width: 70, dataIndex: 'AnnouncementName'},
                {text: '服务器', width: 140, dataIndex: 'AnnouncementService'},
                {text: '渠道', width: 140, dataIndex: 'AnnouncementCreateTime'},
                {text: '状态', width: 140, dataIndex: 'AnnouncementStartTime'},
                {
                    text: "操作",
                    width: 100,
                    dataIndex: 'available',
...
function cancel(buttonId, createTime) {
    Ext.MessageBox.confirm("提示", "您确定取消此公告么?", function (e) {
        ...
        Ext.Ajax.request({
            url: "http://" + window.location.host + "/manageSign/cancel",
            method: 'POST',
            params: {
